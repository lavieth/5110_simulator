...
// General functions
void simulate_IM(bitset<16> op1, bitset<16> op2, uint8_t & num_multiplications, uint8_t & num_additions); // Simulate Iterative Method (IM) 16x16
bitset<16> simulateIM_8x8(bitset<8> op1, bitset<8> op2, uint8_t & num_multiplications, uint8_t & num_additions); // Simulate 8x8 iterative method used with 16x16 version
...
bitset<8> multiplier4x4(bitset<4> op1, bitset<4> op2, uint8_t & num_multiplications); // Simulate 4x4 mulitplier
...
int calculate_timing(const uint8_t multiplications, const uint8_t additions);//displays required cost of 4x4 multipliers and the cost for the subsequent additions 